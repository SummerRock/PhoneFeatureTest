...
public class RippleDrawableUtils {
    ...
    public static Drawable getButtonRippleBackground(@ColorInt int normalColor, float radius) {
        return getCompatRippleDrawable(normalColor, getRippleColor(normalColor), radius);
    }

    public static Drawable getTransparentRippleBackground(float radius) {
    ...
    public static Drawable getCompatRippleDrawable(@ColorInt int normalColor, @ColorInt int pressedColor, float radius) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return new RippleDrawable(ColorStateList.valueOf(getRippleColor(normalColor)),
    ...
    public static Drawable getCompatRippleDrawable(@ColorInt int normalColor, float radius) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return new RippleDrawable(ColorStateList.valueOf(getRippleColor(normalColor)),
    ...
    public static Drawable getContainDisableStatusCompatRippleDrawable(@ColorInt int normalColor, @ColorInt int disableColor, float radius) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return new RippleDrawable(ColorStateList.valueOf(getRippleColor(normalColor)),
    ...
    private static StateListDrawable getStateListDrawable(@ColorInt int normalColor, @ColorInt int pressedColor, @ColorInt int disableColor, float radius) {
        StateListDrawable states = new StateListDrawable();
        if (disableColor != -1) {
            states.addState(new int[]{-android.R.attr.state_enabled}, getShapeDrawable(disableColor, radius));
        }
        if (pressedColor != -1) {
            states.addState(new int[]{android.R.attr.state_pressed}, getShapeDrawable(pressedColor, radius));
            states.addState(new int[]{android.R.attr.state_focused}, getShapeDrawable(pressedColor, radius));
            states.addState(new int[]{android.R.attr.state_activated}, getShapeDrawable(pressedColor, radius));
        }
    ...
    public static Drawable getCompatRippleDrawable(@ColorInt int normalColor, float radiusTopLeft, float radiusTopRight, float radiusBottomLeft, float radiusBottomRight) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return new RippleDrawable(ColorStateList.valueOf(getRippleColor(normalColor)),
                    getStateListDrawable(normalColor, -1, -1, radiusTopLeft, radiusTopRight, radiusBottomLeft, radiusBottomRight),
                    getRippleMask(normalColor, radiusTopLeft, radiusTopRight, radiusBottomLeft, radiusBottomRight));
        } else {
    ...
    private static StateListDrawable getStateListDrawable(@ColorInt int normalColor, @ColorInt int pressedColor, @ColorInt int disableColor,
    ...
    private static GradientDrawable getShapeDrawable(@ColorInt int color,
                                                     float radiusTopLeft,
